...
class blr_vm {
 public:
  blr_vm(unsigned int stack_size, unsigned char *gs)
      : mStackSize(stack_size), mStackPointer(0), mGlobalSpace(gs) {
 ...
 private:
  ...
  uint8_t pop8() {
    uint8_t rval = mStack[mStackPointer];
    --mStackPointer;  // 0
    return rval;
  }
  uint16_t pop16() {
    uint16_t rval = mStack[mStackPointer];
    --mStackPointer;  // 1
    rval <<= 8;
    rval |= mStack[mStackPointer];
    --mStackPointer;  // 0
    return rval;
  }
  uint32_t pop32() {
    uint32_t rval = mStack[mStackPointer];
    --mStackPointer;  // 3
    rval <<= 8;
    rval |= mStack[mStackPointer];
    --mStackPointer;  // 2
  ...
  uint64_t pop64() {
    uint64_t rval = mStack[mStackPointer];
    --mStackPointer;  // 7
    rval <<= 8;
    rval |= mStack[mStackPointer];
    --mStackPointer;  // 6
