...
enum bnf_token_type { bnft_term, bnft_nterm, bnft_yeild, bnft_or };
...
class bnf_parser {
 public:
  bnf_parser(std::string filename)
      : mRoot(""),
        mNull(""),
        mFileName(filename),
        mState(true),
  ...
  bool lex_comment(int char_left) {
    ...
    while ((mPayload[mLexerPointer] != '\n') &&
           (mPayload[mLexerPointer] != '\f') && (char_left > 0)) {
      ++mLexerPointer;
      --char_left;
    }
  ...
  bool lex_root(int char_left) {
    ...
    while (mPayload[mLexerPointer] != '<' && (char_left > 0)) {
      ++mLexerPointer;
      --char_left;
    }
    // Skip <
    ...
    while (mLexerPointer < mPayloadSize) {
      if (mPayload[mLexerPointer] == '#') {
        ...
        if ((mPayload[mLexerPointer - 1] == '\n') ||
            (mPayload[mLexerPointer - 1] == '\f') || (mLexerPointer == 0)) {
  ...
  bool parse_production() {
    ...
    if (find_production((*mTokenCurrent)->mValue) == mProductions.end()) {
      // std::cout << "New production symbol : " << (*mTokenCurrent)->mValue <<
      // std::endl;
      mProductions[(*mTokenCurrent)->mValue] =
          new bnf_production((*mTokenCurrent)->mValue);
    } else {
      // std::cout << "Adding to production symbol : " <<
      // (*mTokenCurrent)->mValue << std::endl;
    }

    // Current production pointer
    bnf_production *prod_pointer = reinterpret_cast<bnf_production *>(
        mProductions[(*mTokenCurrent)->mValue]);
      ...
      while (mTokenCurrent != mTokenList.end()) {
        ...
        // Found "Terminal"
          ...
          if (find_terminal((*mTokenCurrent)->mValue) == mTerminals.end()) {
            // std::cout << "New terminal symbol : " << (*mTokenCurrent)->mValue
            // << std::endl;
        ...
        // Found "Non Terminal"
          ...
          // Is this a new termial symbol
          if (find_production((*mTokenCurrent)->mValue) == mProductions.end()) {
            // std::cout << "New production symbol : " <<
            // (*mTokenCurrent)->mValue << std::endl;
            mProductions[(*mTokenCurrent)->mValue] =
                new bnf_production((*mTokenCurrent)->mValue);
          }
    ...
    if (mIsLexed && mState) {
      ...
      while (mTokenCurrent != end) {
        if ((*mTokenCurrent)->mType == bnft_nterm) {
          ...
          //
          std::cout << "Can't find non-terminal to initiate a production"
                    << std::endl;
          std::cout << "Token type = " << mTokenNames[(*mTokenCurrent)->mType]
                    << std::endl;
  ...
  void output_root() { std::cout << "Root : " << mRoot << std::endl; }
  void output_null() { std::cout << "Null : " << mNull << std::endl; }
  ...
  std::string get_file_name() { return mFileName; }
