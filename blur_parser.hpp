...
class blr_parser {
 public:
  ...
  void using_clause() {
    ...
    if (mState) {
      mState = match(blr_token_using);
    }
    if (mState) {
      mState = match(blr_token_literal);
    }
  ...
  blr_ast_node *class_declaration() {
    ...
    if (mState) {
      mState = match(blr_token_leftbrace);
    }
    if (mState) {
      do {
        if (mState) {
    ...
    if (mState) {
      mState = match(blr_token_rightbrace);
    }
  ...
  blr_ast_node *data_declaration() {
    ...
    if (match(blr_token_var)) {
      ...
      if (mState) {
        mState = match(blr_token_semicolon);
      }
    } else {
      mState = false;
      error();
  ...
  blr_ast_node *data_definition() {
    ...
    if (match(blr_token_var)) {
      ...
      if (match(blr_token_if)) {
        if (mState) {
          mState = match(blr_token_leftpar);
        }
        if (mState) {
          complex_expression();
        }
        if (mState) {
          mState = match(blr_token_rightpar);
        }
      }
  ...
  // The grammar is written so that the function declaration takes only one
  // statement following the argument list The use of {} to group statement is
  // made possible because { statement list } is defined as a compound
  // statement.
  blr_ast_node *fun_declaration() {
    ...
    if (!mState) {
      ...
      fullname += name;
      rval = new blr_ast_node_function_prototype(fullname, mStructureDepth,
                                                 paramtypes, returntype, body);
  ...
  std::vector<blr_ast_node *> params() {
    ...
    // We used this seperation between <params> and <param_list> to test for an
  ...
  blr_ast_node_array *array_definition() {
    ...
    if (mState) {
      mState = match(blr_token_leftbracket);
    }
    if (mState) {
    ...
    if (mState) {
      mState = match(blr_token_rightbracket);
    }
  ...
  // At any level the matched expression can be called upon or assign to
  // something.
  //
  blr_ast_node *augment_expression(blr_ast_node *x) {
    ...
    while (mState && (isnext(blr_token_leftbracket) || isnext(blr_token_dot) ||
      ...
      if (match(blr_token_dot)) {
        std::string name;
        if (mState) {
          match(blr_token_name);
        }
        name = mLastMatchedValue;
        if (mState) {
          match(blr_token_leftpar);
        }
        std::vector<blr_ast_node *> mArguments = args();
        if (mState) {
          match(blr_token_rightpar);
        }
        if (mState) {
          rval =
              new blr_ast_node_expression_call_member(name, rval, mArguments);
        }
      }
      // Access
        ...
        if (mState) {
          rval = new blr_ast_node_expression_call_member("_@_ACCESS__", rval,
                                                         mArguments);
        }
        if (mState) {
          match(blr_token_rightbracket);
        }
      }
      // Assignment
        ...
        if (mState) {
          std::vector<blr_ast_node *> mArguments;
          mArguments.push_back(complex_expression());
          rval = new blr_ast_node_expression_call_member("_@_ASSIGN__", rval,
                                                         mArguments);
        }
      }
    }
  ...
  blr_ast_node *complex_expression() {
    ...
    // The result of this expression is used as the context and is the *this*
    // object
    if (mState) {
      rval = augment_expression(simple_expression());
    }
  ...
  blr_ast_node *simple_expression() {
    ...
    if (mState) {
      rval = augment_expression(simple_term());
    }
  ...
  blr_ast_node *simple_term() {
    ...
    if (mState) {
      rval = augment_expression(simple_factor());
    }
  ...
  blr_ast_node *simple_factor() {
    ...
    if (mState) {
      ...
      if (matchun()) {
        ...
        if (mState) {
          rval = new blr_ast_node_expression_unary_op(mLastMatchedTokenType,
                                                      simple_term());
        }
      }
      // Function Call
        ...
        rval = new blr_ast_node_expression_integer_number_literal(
            atoi(mLastMatchedValue.c_str()));
        ...
        // Empty expression
        // If we don't find anything we need to exit without issues, it means it
        // should match higher priority operations
      }
    }
  ...
  std::vector<blr_ast_node *> args() {
    ...
    // We used this seperation between <args> and <args_list> to test for an
    // empty list Like this: ()
  ...
  std::vector<blr_ast_node *> args_list() {
    ...
    do {
      if (mState) {
        curr_arg = complex_expression();
      }
      if (mState) {
        rval.push_back(curr_arg);
      }
    } while (mState && match(blr_token_comma));
  ...
  blr_ast_node *if_statement() {
    ...
    if (!mState) {
      ...
      rval =
          new blr_ast_node_statement_if(condition, ifstatement, elsestatement);
    }
    return rval;
  }
  ...
  blr_ast_node *for_statement() {
    ...
    if (!mState) {
      ...
      rval = new blr_ast_node_statement_for(init, condition, update,
                                            loopstatement);
    }
  ...
  blr_ast_node *foreach_statement() {
    ...
    if (mState) {
      mState = match(blr_token_foreach);
    }
    if (mState) {
      mState = match(blr_token_leftpar);
    }
    ...
    if (!mState) {
      ...
      rval =
          new blr_ast_node_statement_foreach(element, container, loopstatement);
    }
    return rval;
  }
  ...
  bool type_exists(std::string tsname) {
    ...
    if (std::find(mDeclaredTypeList.begin(), mDeclaredTypeList.end(), tsname) !=
        mDeclaredTypeList.end()) {
      rval = true;
    }
  ...
  bool isnext(BLR_TOKEN_TYPE tt) {
    ...
    // std::cout << "IS Next : " <<
    // (*mpTokenTypeNames)[(*mLookAheadSymbol)->mType]
    //          << std::endl;
  ...
  bool matchbin1() {
    ...
    if ((tt == blr_token_plus) || (tt == blr_token_minus) ||
        (tt == blr_token_lesser) || (tt == blr_token_greater) ||
        (tt == blr_token_lesserequal) || (tt == blr_token_greaterequal) ||
        (tt == blr_token_equal) || (tt == blr_token_notequal) ||
        (tt == blr_token_or) || (tt == blr_token_and) ||
        (tt == blr_token_xor) || (tt == blr_token_bwor) ||
        (tt == blr_token_bwand) || (tt == blr_token_leftshift) ||
  ...
  bool matchbin2() {
    bool rval = false;
    BLR_TOKEN_TYPE tt = (**mLookAheadSymbol).mType;
    if ((tt == blr_token_slash) || (tt == blr_token_star) ||
        (tt == blr_token_modulo)) {
      std::cout << "   Match In   : " << (*mpTokenTypeNames)[tt] << " -> "
                << (**mLookAheadSymbol).mValue << std::endl;
      mLastMatchedTokenType = (**mLookAheadSymbol).mType;
  ...
  bool matchun() {
    ...
    BLR_TOKEN_TYPE tt = (**mLookAheadSymbol).mType;
    if ((tt == blr_token_plus) || (tt == blr_token_minus) ||
        (tt == blr_token_not) || (tt == blr_token_bwnot)) {
      std::cout << "   Match In   : " << (*mpTokenTypeNames)[tt] << " -> "
                << (**mLookAheadSymbol).mValue << std::endl;
      mLastMatchedTokenType = (**mLookAheadSymbol).mType;
      ++mLookAheadSymbol;
  ...
  bool error() {
    if (!mReported) {
      std::cout << "Error at line - " << (**mLookAheadSymbol).mLineNumber
                << std::endl;
      mReported = true;
    }
  }
  ...
  void output_function(blr_ast_node *x) {
    if (blr_ast_node_function_prototype *p =
  ...
  void output_expression(blr_ast_node *x) {
    ...
    if (blr_ast_node_expression_integer_number_literal *p =
      ...
      std::cout << " ";
    } else if (blr_ast_node_expression_variable *p =
                   dynamic_cast<blr_ast_node_expression_variable *>(x)) {
      std::cout << "V:" << p->mValue << " ";
    } else if (blr_ast_node_expression_call *p =
                   dynamic_cast<blr_ast_node_expression_call *>(x)) {
      ...
      std::cout << ")";
    } else if (blr_ast_node_expression_call_member *p =
                   dynamic_cast<blr_ast_node_expression_call_member *>(x)) {
      ...
      std::cout << ")";
    } else if (blr_ast_node_expression_defered_call *p =
                   dynamic_cast<blr_ast_node_expression_defered_call *>(x)) {
      // This should find the expression call if branch
      output_expression(p->mCall);
    } else if (blr_ast_node_expression_binary_op *p =
                   dynamic_cast<blr_ast_node_expression_binary_op *>(x)) {
      output_token(p->mOp);
      std::cout << "(";
      ...
      std::cout << ")";
    } else if (blr_ast_node_expression_unary_op *p =
                   dynamic_cast<blr_ast_node_expression_unary_op *>(x)) {
      output_token(p->mOp);
      std::cout << " ";
      output_expression(p->mExp);
    } else if (blr_ast_node_expression_complex *p =
                   dynamic_cast<blr_ast_node_expression_complex *>(x)) {
      std::cout << "CE( ";
  ...
  void output_statement(blr_ast_node *x) {
    if (blr_ast_node_statement_if *p =
            dynamic_cast<blr_ast_node_statement_if *>(x)) {
      std::cout << "IF ( ";
      ...
      std::cout << " } ";
      std::cout << std::endl;
    } else if (blr_ast_node_statement_while *p =
                   dynamic_cast<blr_ast_node_statement_while *>(x)) {
      std::cout << "WHILE ( ";
      output_expression(p->mCondition);
      std::cout << " ) { ";
      ...
      std::cout << " } ";
      std::cout << std::endl;
    } else if (blr_ast_node_statement_for *p =
                   dynamic_cast<blr_ast_node_statement_for *>(x)) {
      std::cout << "FOR ( ";
      ...
      std::cout << " } ";
    } else if (blr_ast_node_statement_foreach *p =
                   dynamic_cast<blr_ast_node_statement_foreach *>(x)) {
      std::cout << "FOREACH ( ";
      std::cout << p->mElement << " in ";
      std::cout << p->mContainer << " ) { ";
      output_expression(p->mLoopStatement);
      std::cout << " } ";
    } else if (blr_ast_node_statement_break *p =
                   dynamic_cast<blr_ast_node_statement_break *>(x)) {
      std::cout << "BREAK" << std::endl;
    } else if (blr_ast_node_statement_return *p =
                   dynamic_cast<blr_ast_node_statement_return *>(x)) {
      std::cout << "RETURN ";
      output_expression(p->mRval);
    } else if (blr_ast_node_statement_compound *p =
                   dynamic_cast<blr_ast_node_statement_compound *>(x)) {
      mIndentationLevel += 3;
      std::cout << " { " << std::endl;
      ...
      std::cout << " } ";
                   ...
                   dynamic_cast<blr_ast_node_variable_definition *>(x)) {
  ...
  void output_all_functions() {
    std::cout
        << "----------------------------------------------------------------"
        << std::endl;
    std::cout << "Output " << mASTFunctions.size() << " function(s)"
              << std::endl;
  ...
  void output_all_declaredtypenames() {
    std::cout
        << "----------------------------------------------------------------"
        << std::endl;
    std::cout << "Output " << mDeclaredTypeList.size() << " declared type names"
              << std::endl;
  ...
  // Access Functions
  // ----------------------------------------------------------------------
  bool get_state() { return mState; }

  std::list<blr_ast_node *> *get_struct_ast_list() { return &mASTStructs; }
 ...
 private:
  ...
  bool IsEnd() { return (mLookAheadSymbol == mpTokenList->end()); }
