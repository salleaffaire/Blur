...
class blr_ast_node_expression_integer_number_literal
    : public blr_ast_node_expression {
 public:
  virtual ~blr_ast_node_expression_integer_number_literal() {}
  int64_t mValue;
  blr_ast_node_expression_integer_number_literal(int64_t value)
      : mValue(value) {}
};

class blr_ast_node_expression_float_number_literal
    : public blr_ast_node_expression {
 public:
...
class blr_ast_node_expression_call : public blr_ast_node_expression {
 public:
  ...
  blr_ast_node_expression_call(std::string callee,
...
class blr_ast_node_expression_call_member : public blr_ast_node_expression {
 public:
  ...
  std::vector<blr_ast_node *> mArgs;
  blr_ast_node_expression_call_member(std::string callee,
                                      blr_ast_node *calledon,
                                      std::vector<blr_ast_node *> a)
...
class blr_ast_node_expression_defered_call : public blr_ast_node_expression {
 public:
  virtual ~blr_ast_node_expression_defered_call() { delete mCall; }
  blr_ast_node *mCall;
  blr_ast_node_expression_defered_call(blr_ast_node *call) : mCall(call) {}
};
...
class blr_ast_node_expression_binary_op : public blr_ast_node_expression {
 public:
  ...
  blr_ast_node_expression_binary_op(BLR_TOKEN_TYPE op, blr_ast_node *l,
                                    blr_ast_node *r)
...
class blr_ast_node_expression_unary_op : public blr_ast_node_expression {
 public:
  virtual ~blr_ast_node_expression_unary_op() { delete mExp; }
  BLR_TOKEN_TYPE mOp;
  blr_ast_node *mExp;
  blr_ast_node_expression_unary_op(BLR_TOKEN_TYPE op, blr_ast_node *e)
      : mOp(op), mExp(e) {}
};
...
class blr_ast_node_array : public blr_ast_node {
 public:
  virtual ~blr_ast_node_array() { delete mOf; }
  std::string mSize;
...
class blr_ast_node_list : public blr_ast_node {
 public:
  virtual ~blr_ast_node_list() { delete mOf; }
  std::string mName;
  blr_ast_node *mOf;
...
class blr_ast_node_variable_declaration : public blr_ast_node {
 public:
  virtual ~blr_ast_node_variable_declaration() { delete mType; }
  std::string mName;
  blr_ast_node *mType;
...
class blr_ast_node_variable_definition : public blr_ast_node {
 public:
  ...
  blr_ast_node_variable_definition(std::string name, blr_ast_node *type,
                                   blr_ast_node *cond)
...
class blr_ast_node_function_prototype : public blr_ast_node {
 public:
  ...
  blr_ast_node_function_prototype(std::string name,
                                  std::vector<std::string> memberof,
                                  std::vector<blr_ast_node *> paramtypes,
                                  blr_ast_node *rt, blr_ast_node *st)
      : mName(name),
        mMemberOf(memberof),
...
class blr_ast_node_statement_if : public blr_ast_node_statement {
 public:
  ...
  blr_ast_node_statement_if(blr_ast_node *cond, blr_ast_node *ifs,
                            blr_ast_node *elses)
...
class blr_ast_node_statement_for : public blr_ast_node_statement {
 public:
  ...
  blr_ast_node_statement_for(blr_ast_node *init, blr_ast_node *cond,
                             blr_ast_node *update, blr_ast_node *loopstatement)
      : mInit(init),
        mCondition(cond),
        mUpdate(update),
...
class blr_ast_node_statement_foreach : public blr_ast_node_statement {
 public:
  ...
  blr_ast_node_statement_foreach(std::string elem, std::string container,
...
class blr_ast_node_statement_return : public blr_ast_node_statement {
 public:
  virtual ~blr_ast_node_statement_return() { delete mRval; }
  blr_ast_node *mRval;
